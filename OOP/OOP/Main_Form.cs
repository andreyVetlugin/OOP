...
namespace OOP
{
    public class TableManager
    {
        ...
        private List<TableFiller> getNumericTableFillers()//!!! заменить все проверки данных на функцию
        {
            ...
            {
                int first_param, second_param;
                if (!int.TryParse(currentTable[1, 0].Value.ToString(), out first_param))
                    return;
                if (!int.TryParse(currentTable[2, 0].Value.ToString(), out second_param))
                    return;
                if (first_param <= 0 || second_param <= 0)
                    return;
                currentTable[3, 0].Value = (double)first_param / second_param;
            });
    ...
    public partial class Main_Form : Form
    {
        ...
        {
            InitializeComponent();
            // так же разделить по функциям, может добавить в менеджер это заполнение 
